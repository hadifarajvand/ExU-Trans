...
class DownBlock(nn.Module):
    """Downsampling block with max pool and convolution."""
    def __init__(self, in_ch, out_ch):
        super().__init__()
...
class SEMHABlock(nn.Module):
    """Squeeze-Excitation Multi-Head Attention block."""
...
class ContextualSelfAttention(nn.Module):
    """Contextual spatial and channel attention."""
...
class BivariateFusion(nn.Module):
    """Bivariate fusion of local and global features."""
...
class ExUTransLite(nn.Module):
    """ExU-Trans Lite: Dual-encoder with U-Net + ViT-style attention."""
    ...
    def forward(self, x):
        ...
        return out, {
            ...
            "attention_map": self.sem2.attn_map,
...
def create_model(in_ch=4, out_ch=1, patch_size=16, device='cpu'):
    """Create and move model to device."""
    model = ExUTransLite(in_ch=in_ch, out_ch=out_ch, patch_size=patch_size).to(device)
    return model
